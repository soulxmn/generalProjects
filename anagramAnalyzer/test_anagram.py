...
class testAnagramSolver(unittest.TestCase):
  ...
  def testOneWordAnagram(self: "testAnagramSolver") -> None:
    ...
    self.assertEqual(testOWA, [['hello']], 'Output incorrect!') 
  ...
  def testNoLimit(self: "testAnagramSolver") -> None:
    ...
    s = 'office key'    
    maxWords = 0
    h = x.generateAnagrams(s, maxWords) 
    # Removing data copied from previous test outside this class.
    h.remove(['hello'])
    self.assertEqual(h, [['eke', 'icy', 'off'], \
                       ['eke', 'off', 'icy'], \
                       ['ice', 'key', 'off'], \
                       ['ice', 'off', 'key'], \
                       ['icy', 'eke', 'off'], \
  ...
  def testLimit(self: "testAnagramSolver") -> None:
    ...
    maxWords = 2
    h = x.generateAnagrams(s, maxWords)
    # Removing data copied over from previous test outside this class.
    h.pop(0)
    self.assertEqual(h, [['key', 'office'], \
                        ['office', 'key']], 'Output incorrect!') 
  ...
  def testOneLetterWord(self: "testAnagramSolver") -> None:
    ...
    anagramDict.append('I')
    x = AnagramSolver(sorted(anagramDict))
